...
Color FrameBuffer::get_pixel(uint x, uint y) const { return buffer.at(pixel_index(x, y)); }

inline uint FrameBuffer::pixel_index(uint x, uint y) const { return (y * width) + x; }
