...
std::unique_ptr<CartridgeInfo> get_info(const std::array<u8, MAX_ROM_SIZE>& rom) {
    ...
    info->type = get_cartridge_type(rom);
...
CartridgeType get_cartridge_type(const std::array<u8, MAX_ROM_SIZE>& rom) {
    switch (rom[0x147]) {
    case 0x00:
    case 0x08:
    case 0x09:
    ...
    default:
        log_error("Unknown cartridge type");
        return CartridgeType::Unknown;
    }
}
...
std::string get_title(const std::array<u8, MAX_ROM_SIZE>& rom) {
