...
PPU::PPU(Gameboy& inGb, Options& inOptions) :
    gb(inGb),
...
u8 PPU::read(const Address& address) {
    return video_ram.at(address.value());
}

void PPU::write(const Address& address, u8 value) {
    video_ram.at(address.value()) = value;
}

void PPU::tick(Cycles cycles) {
...
bool PPU::display_enabled() const { return check_bit(control_byte, 7); }
bool PPU::window_tile_map() const { return check_bit(control_byte, 6); }
bool PPU::window_enabled() const { return check_bit(control_byte, 5); }
bool PPU::bg_window_tile_data() const { return check_bit(control_byte, 4); }
bool PPU::bg_tile_map_display() const { return check_bit(control_byte, 3); }
bool PPU::sprite_size() const { return check_bit(control_byte, 2); }
bool PPU::sprites_enabled() const { return check_bit(control_byte, 1); }
bool PPU::bg_enabled() const { return check_bit(control_byte, 0); }

void PPU::write_scanline(u8 current_line) {
    if (!display_enabled()) {
        return;
    }
...
void PPU::draw_bg_line(uint current_line) {
...
void PPU::draw_window_line(uint current_line) {
    // Note: tileset two uses signed numbering to share half the tiles with tileset 1
    bool use_tile_set_zero = bg_window_tile_data();
...
void PPU::draw_sprite(const uint sprite_n) {
...
GBColor PPU::get_pixel_from_line(u8 byte1, u8 byte2, u8 pixel_index) {

    u8 color_u8 = static_cast<u8>((bit_value(byte2, 7 - pixel_index) << 1) | bit_value(byte1, 7 - pixel_index));
    return get_color(color_u8);
}

bool PPU::is_on_screen_x(u8 x) { return x < GAMEBOY_WIDTH; }

bool PPU::is_on_screen_y(u8 y) { return y < GAMEBOY_HEIGHT; }

bool PPU::is_on_screen(u8 x, u8 y) { return is_on_screen_x(x) && is_on_screen_y(y); }

Palette PPU::load_palette(Register8& palette_register) {
...
Color PPU::get_color_from_palette(GBColor color, const Palette& palette) {
    switch (color) {
    case GBColor::Color0:
...
Color PPU::get_real_color(u8 pixel_value) {
    switch (pixel_value) {
    case 0:
        return Color::White;
    case 1:
...
void PPU::register_vblank_callback(const vblank_callback_t& _vblank_callback) {
