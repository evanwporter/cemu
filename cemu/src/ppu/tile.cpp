...
Tile::Tile(Address& tile_address, MMU& mmu, uint size_multiplier) {
    ...
    for (uint tile_line = 0; tile_line < TILE_HEIGHT_PX * size_multiplier; tile_line++) {
        ...
        const u8 pixels_1 = mmu.read(line_start);
        const u8 pixels_2 = mmu.read(line_start + 1);
...
GBColor Tile::get_pixel(uint x, uint y) const { return buffer[pixel_index(x, y)]; }

std::vector<u8> Tile::get_pixel_line(u8 byte1, u8 byte2) {
...
inline uint Tile::pixel_index(uint x, uint y) { return (y * TILE_HEIGHT_PX) + x; }
