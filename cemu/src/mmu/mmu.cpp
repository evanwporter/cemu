...
u8 MMU::read(const Address& address) const {
    ...
    // VRAM
    if (address <= 0x9FFF) {
        return gb.ppu->read(address.value() - 0x8000);
    }
...
u8 MMU::read_io(const Address& address) const {
    ...
    case 0xFF41:
        return gb.ppu->lcd_status.value();

    case 0xFF42:
        return gb.ppu->scroll_y.value();
    ...
    case 0xFF47:
        return gb.ppu->bg_palette.value();

    case 0xFF48:
        return gb.ppu->sprite_palette_0.value();

    case 0xFF49:
        return gb.ppu->sprite_palette_1.value();

    case 0xFF4A:
        return gb.ppu->window_y.value();

    case 0xFF4B:
        return gb.ppu->window_x.value();
...
u8 MMU::unmapped_io_read(const Address& address) const {
...
void MMU::write(const Address& address, const u8 byte) {
    ...
    if (address.in_range(0x8000, 0x9FFF)) {
        gb.ppu->write(address.value() - 0x8000, byte);
        return;
    }
...
void MMU::write_io(const Address& address, const u8 byte) {
    ...
    // Switch on LCD
    case 0xFF40:
        gb.ppu->control_byte = byte;
        return;

    case 0xFF41:
        gb.ppu->lcd_status.set(byte);
        return;

    // Vertical Scroll Register
    case 0xFF42:
        gb.ppu->scroll_y.set(byte);
        return;

    // Horizontal Scroll Register
    case 0xFF43:
        gb.ppu->scroll_x.set(byte);
        return;
    ...
    case 0xFF47:
        gb.ppu->bg_palette.set(byte);
        log_trace("Set ppu palette: 0x%x", byte);
        return;

    case 0xFF48:
        gb.ppu->sprite_palette_0.set(byte);
        log_trace("Set sprite palette 0: 0x%x", byte);
        return;

    case 0xFF49:
        gb.ppu->sprite_palette_1.set(byte);
        log_trace("Set sprite palette 1: 0x%x", byte);
        return;

    case 0xFF4A:
        gb.ppu->window_y.set(byte);
        return;

    case 0xFF4B:
        gb.ppu->window_x.set(byte);
        return;
