...
inline bool check_bit(const u8 value, const u8 bit) { return (value & (1 << bit)) != 0; }

inline u8 bit_value(const u8 value, const u8 bit) { return (value >> bit) & 1; }
...
inline u8 set_bit(const u8 value, const u8 bit, bool bit_on) {
    return bit_on
        ? set_bit(value, bit)
        : clear_bit(value, bit);
}
