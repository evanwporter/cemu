...
static const fs::path kTestDir = fs::path(TEST_DIR) / "sm83/v1";
...
static void verify_registers(const Vcpu_top& top, const json& expected, const std::string& test_name) {
    ...
    EXPECT_EQ(get_u16(regs->__PVT__sph, regs->__PVT__spl), read_u16(expected["sp"]))
        << "Test: " << test_name;
    EXPECT_EQ(get_u16(regs->__PVT__pch, regs->__PVT__pcl) - 1, read_u16(expected["pc"]))
        << "Test: " << test_name;
}
...
class SM83_GameboyOpcodeTest : public ::testing::TestWithParam<fs::path> { };

TEST_P(SM83_GameboyOpcodeTest, Run) {
    run_single_file(GetParam());
}

INSTANTIATE_TEST_SUITE_P(
    CPUTests,
    SM83_GameboyOpcodeTest,
    ::testing::ValuesIn(collect_files_in_directory(
        kTestDir,
        ".json")),
    [](const ::testing::TestParamInfo<fs::path>& info) {
        return get_test_name(info);
    });
